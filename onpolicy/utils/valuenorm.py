...
class ValueNorm(nn.Module):
    ...
    @torch.no_grad()
    def update(self, input_vector, agent_id=None):
    ...
    def normalize(self, input_vector, agent_id=None):
    ...
    def denormalize(self, input_vector, agent_id=None):
...
class MultiVN:
    ...
    def __init__(self, vn_list: List[nn.Module]) -> None:
        self.vn_list = vn_list
        self.num_agents = len(self.vn_list)
    ...
    def reset_parameters(self,
        ...
        for i in agent_id:
            self.vn_list[i].reset_parameters()
    ...
    def update(self,
        ...
        if agent_id is None:
            ...
            agent_id = np.array([agent_id])
        # print(agent_id, sys._getframe().f_lineno, __file__)
        input_vector = input_vector.reshape(-1, len(agent_id),
                                            input_vector.shape[-1])
        for i, a_i in enumerate(agent_id):
            self.vn_list[a_i].update(input_vector[..., i, :])

    def normalize(
            self,
            input_vector: Union[np.ndarray, torch.Tensor],
            agent_id: Optional[Union[int, Iterable]] = None) -> torch.Tensor:
        if agent_id is None:
            agent_id = np.arange(self.num_agents)
        elif not np.iterable(agent_id):
            agent_id = np.array([agent_id])
        origin_shape = input_vector.shape[:-1]
        _input_vector = []
        input_vector = input_vector.reshape(-1, len(agent_id),
                                            input_vector.shape[-1])
        for i, a_i in enumerate(agent_id):
            _input_vector.append(self.vn_list[a_i].normalize(input_vector[:,
                                                                          i]))
        _input_vector = torch.stack(_input_vector, dim=1)
        _input_vector = _input_vector.reshape(*origin_shape,
                                              _input_vector.shape[-1])
    ...
    def denormalize(
            self,
            input_vector: Union[np.ndarray, torch.Tensor],
            agent_id: Optional[Union[int, Iterable]] = None) -> np.ndarray:
        if agent_id is None:
            agent_id = np.arange(self.num_agents)
        elif not np.iterable(agent_id):
            agent_id = np.array([agent_id])
        ...
        input_vector = input_vector.reshape(-1, len(agent_id),
                                            input_vector.shape[-1])
        for i, a_i in enumerate(agent_id):
            _input_vector.append(self.vn_list[a_i].denormalize(
                input_vector[:, i]))
        _input_vector = np.stack(_input_vector, axis=1)
        _input_vector = _input_vector.reshape(*origin_shape,
                                              _input_vector.shape[-1])
        return _input_vector